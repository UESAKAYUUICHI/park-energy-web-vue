...
import { alarmEvents, alarmRules, alarmSummary, createAlarmWorkOrder, dealAlarm, listResource, saveAlarmRule } from '@/api/platform'
...
async function createWorkOrder(row: RecordRow) { try { const order = await createAlarmWorkOrder(row.id); selected.value = null; await router.push(`/operations/work-orders?workOrderId=${order.id}`) } catch (e) { error.value = e instanceof Error ? e.message : '创建告警工单失败' } }
...
<template>
  <section class="view-page" :class="mode === 'alarm-rules' ? 'alarm-rules-page' : 'alarm-workbench-page'">
    ...
    <AppDrawer :open="Boolean(selected)" title="告警详情" @update:open="(open) => { if (!open) selected = null }"><dl class="detail-grid"><template v-for="item in detailItems" :key="item[0]"><dt>{{ item[0] }}</dt><dd>{{ item[1] }}</dd></template></dl><div class="drawer-actions alarm-drawer-actions"><button class="quiet" @click="goDevice()">设备档案</button><button class="quiet" @click="goEnergy()">实时监控</button><button class="quiet" @click="goCommands()">指令追踪</button><button v-if="selected && Number(selected.deal_status) === 0 && session.can('ops:workorder:create')" class="quiet" @click="createWorkOrder(selected)">转运维工单</button><button v-if="selected && Number(selected.deal_status) === 0 && session.can('alarm:event:deal')" class="primary" @click="openDeal(selected)">处置告警</button></div></AppDrawer>
