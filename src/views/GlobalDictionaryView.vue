...
<template>
  <section class="view-page dictionary-page">
    <header class="view-head"><div><p class="eyebrow">GLOBAL DICTIONARY</p><h1>{{ title }}</h1><p>{{ isPoint ? '定义实时数据语义；协议和解析路径只在产品模板中配置。' : '定义产品可选的固定规格值；固定值由产品模板绑定，不参与实时解析。' }}</p></div><button class="icon-btn" title="刷新" @click="load"><RefreshCw :size="16" /></button></header>
