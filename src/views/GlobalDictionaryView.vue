...
function showField(key: string, value: unknown, row?: RecordRow) { return displayValue(key, value, row) }
...
const details = computed(() => {
  ...
  return fields.map(([label,...keys]) => ({ label, value: label === '是否必填' ? showField('required', val(node, 'required'), node) : label === '固定值数量' ? `${children(node).length} 项` : showField(String(keys[0]), val(node, ...keys), node) }))
})
let loadRequest = 0
