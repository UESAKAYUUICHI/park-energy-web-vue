...
<template>
  <section class="view-page">
    ...
    <AppDialog v-model:open="previewDialog" title="账单试算结果" description="请核对试算明细与金额；点击“确认生成账单”后才会调用生成接口。" confirm-text="确认生成账单" :saving="calculating" @submit="createBill"><div class="preview-dialog"><div class="amount">¥ {{ Number(preview?.totalAmount || 0).toLocaleString('zh-CN', { minimumFractionDigits: 2 }) }}</div><p>本次试算共 {{ previewDetails.length }} 条计费明细。</p></div><AppDataTable title="试算明细" :columns="detailColumns" :rows="previewDetails" empty-text="本次试算没有可计费明细。" /></AppDialog>
    ...
    <AppDialog v-model:open="paymentDialog" title="账单缴费" description="后端仅接受与应收总额相等的全额缴费。" @submit="pay"><div class="dialog-fields"><label class="dialog-field"><span>缴费金额</span><input v-model="payment.payAmount" type="number"></label><label class="dialog-field"><span>支付方式</span><select v-model="payment.payWay"><option>转账</option><option>现金</option><option>线上支付</option></select></label><label class="dialog-field full"><span>备注</span><textarea v-model="payment.remark"></textarea></label></div></AppDialog>
  </section>
</template>
