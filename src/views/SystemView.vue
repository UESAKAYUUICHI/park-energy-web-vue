...
import { audit, rbac, rbacPage, resourceOptions } from '@/api/platform'
...
function openRelation(target: RecordRow, modeValue: 'password' | 'roles' | 'permissions' | 'scopes') { relationTargetId.value = target.id; relationMode.value = modeValue; relationTitle.value = ({ password: '重置登录密码', roles: '分配用户角色', permissions: '分配角色权限', scopes: '配置组织数据范围' } as const)[modeValue]; relationText.value = modeValue === 'scopes' ? '1,SELF' : ''; if (modeValue === 'roles') relationSelectedIds.value = (Array.isArray(relations.value.userRoles) ? relations.value.userRoles : []).filter((item) => String(item.userId ?? item.user_id) === String(target.id)).map((item) => Number(item.roleId ?? item.role_id)); else if (modeValue === 'permissions') relationSelectedIds.value = (Array.isArray(relations.value.rolePermissions) ? relations.value.rolePermissions : []).filter((item) => String(item.roleId ?? item.role_id) === String(target.id)).map((item) => Number(item.permissionId ?? item.permission_id)); else relationSelectedIds.value = []; relationDialog.value = true }
async function saveRelation() { if (!relationTargetId.value) return; try { let path = ''; let body: RecordRow = {}; if (relationMode.value === 'password') { path = `users/${relationTargetId.value}/password`; body = { password: relationText.value } } else if (relationMode.value === 'roles') { path = `users/${relationTargetId.value}/roles`; body = { ids: relationSelectedIds.value } } else if (relationMode.value === 'permissions') { path = `roles/${relationTargetId.value}/permissions`; body = { ids: relationSelectedIds.value } } else { path = `users/${relationTargetId.value}/org-scopes`; body = { scopes: relationText.value.split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((line) => { const [orgId, scopeMode] = line.split(','); return { orgId: Number(orgId), scopeMode: (scopeMode || 'SELF').trim() } }) } } await rbac(path, { method: relationMode.value === 'scopes' ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); relationDialog.value = false; await load() } catch (e) { error.value = e instanceof Error ? e.message : '关联配置保存失败' } }
...
<template>
  <section class="view-page system-page"><header class="view-head"><div><p class="eyebrow">SYSTEM & ACCESS CONTROL</p><h1>{{ title }}</h1></div></header>
    ...
    <AppDialog v-model:open="dialog" :title="editingId ? `编辑${title}` : `新增${title}`" @submit="save"><div class="dialog-fields"><label v-for="field in configuration?.fields || []" :key="field" class="dialog-field" :class="{ full: ['remark'].includes(field) }"><span>{{ fieldLabel(field) }}</span><textarea v-if="field === 'remark'" v-model="form[field]"></textarea><AppSelect v-else-if="systemFieldOptions(field).length" v-model="form[field]"><option v-for="option in systemFieldOptions(field)" :key="String(option.value)" :value="option.value">{{ option.label }}</option></AppSelect><input v-else v-model="form[field]" :type="inputType(field)"></label></div></AppDialog>
    <AppDialog v-model:open="relationDialog" :title="relationTitle" @submit="saveRelation"><div class="dialog-fields"><label v-if="relationMode === 'password'" class="dialog-field full"><span>新密码</span><input v-model="relationText" type="password" placeholder="请输入新密码"></label><label v-else-if="relationMode === 'scopes'" class="dialog-field full"><span>组织范围</span><textarea v-model="relationText" spellcheck="false" placeholder="组织ID,SELF"></textarea></label><section v-else class="relation-picker full"><header><b>{{ relationMode === 'roles' ? '选择角色' : '选择权限' }}</b><small>支持多选，不需要手动填写编号</small></header><label v-for="item in (relationMode === 'roles' ? relationRoles : relationPermissions)" :key="String(item.id)" class="relation-picker-item"><input v-model="relationSelectedIds" type="checkbox" :value="Number(item.id)"><span><b>{{ item.roleName || item.role_name || item.permName || item.perm_name }}</b><small>{{ item.roleCode || item.role_code || item.permCode || item.perm_code || item.id }}</small></span></label></section></div></AppDialog>
