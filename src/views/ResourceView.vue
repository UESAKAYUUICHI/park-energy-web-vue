...
import { Eye, Pencil, RefreshCw, Trash2 } from '@lucide/vue'
import { resourceSchemas, type ResourceSchema } from '@/config/resourceSchemas'
import { createResource, getResource, listResource, removeResource, updateResource } from '@/api/platform'
...
const detailEntries = computed(() => detailRow.value ? Object.entries(detailRow.value).map(([key, value]) => [key, displayValue(key, value, detailRow.value as RecordRow)] as const) : [])
...
const showTypeFilter = computed(() => ['devices', 'point-definitions'].includes(schemaResource.value))
const showAccountFilter = computed(() => ['rules'].includes(schemaResource.value))
async function load() {
  const currentSchema = requireSchema()
  if (!currentSchema) return
  loading.value = true
  error.value = ''
  try {
    const page = await listResource(currentSchema.area, currentSchema.resource, { pageNum: pageNum.value, pageSize, keyword: keyword.value, orgId: orgId.value || undefined, gatewayId: gatewayId.value || undefined, deviceTypeId: deviceTypeId.value || undefined, accountId: accountId.value || undefined })
...
function openCreate() { const currentSchema = requireSchema(); if (!currentSchema) return; editingId.value = null; copy(form, structuredClone(currentSchema.defaults)); dialog.value = true }
function openEdit(row: RecordRow) { const currentSchema = requireSchema(); if (!currentSchema) return; editingId.value = row.id; copy(form, Object.fromEntries(currentSchema.fields.map((field) => [field.key, row[field.key] ?? '']))); dialog.value = true }
async function save() {
  const currentSchema = requireSchema()
  if (!currentSchema) return
  saving.value = true
  try {
    const body = Object.fromEntries(Object.entries(form).filter(([, value]) => value !== '')) as RecordRow
    if (editingId.value) await updateResource(currentSchema.area, currentSchema.resource, editingId.value, body)
    else await createResource(currentSchema.area, currentSchema.resource, body)
    dialog.value = false
    await load()
  } catch (e) {
...
async function confirmDelete() {
  const currentSchema = requireSchema()
  if (!currentSchema || !deletingRow.value) return
  deleting.value = true
  try {
    await removeResource(currentSchema.area, currentSchema.resource, deletingRow.value.id)
...
<template>
  <section v-if="schemaReady" class="view-page resource-page"><header v-if="!props.embedded" class="view-head"><div><p class="eyebrow">{{ schemaArea.toUpperCase() }} CONFIGURATION</p><h1>{{ schemaTitle }}</h1></div></header>
    ...
    <AppDialog v-model:open="detailDialog" :title="`${schemaTitle}详情`" eyebrow="BUSINESS RECORD" hide-actions dialog-class="business-record-dialog"><dl class="detail-grid dialog-detail-grid"><template v-for="([key, value]) in detailEntries" :key="key"><dt>{{ resourceLabel(key) }}</dt><dd>{{ value }}</dd></template></dl></AppDialog>
  ...
  <section v-else class="view-page resource-page">
