...
async function loadLookups() { try { const [orgs, gateways, types, devices, accounts] = await Promise.all([listResource('archive', 'orgs', { pageSize: 500 }), listResource('archive', 'gateways', { pageSize: 500 }), listResource('archive', 'device-types', { pageSize: 500 }), listResource('archive', 'devices', { pageSize: 500 }), listResource('billing', 'accounts', { pageSize: 500 })]); orgOptions.value = orgs.records; gatewayOptions.value = gateways.records; typeOptions.value = types.records; deviceOptions.value = devices.records; accountOptions.value = accounts.records } catch { /* lookups are optional; primary table still reports its own request failure */ } }
...
<template>
  <section v-if="schema" class="view-page resource-page"><header class="view-head"><div><p class="eyebrow">{{ schema.area.toUpperCase() }} CONFIGURATION</p><h1>{{ schema.title }}</h1><p>{{ schema.description }}</p></div></header>
    ...
    <AppDialog v-model:open="parseDialog" title="协议映射解析测试" @submit="runParseTest"><div class="dialog-fields"><label class="dialog-field"><span>设备类型 ID</span><input v-model="parseForm.deviceTypeId" type="number"></label><label class="dialog-field full"><span>JSON 示例报文</span><textarea v-model="parseForm.samplePayload" spellcheck="false"></textarea></label></div></AppDialog>
