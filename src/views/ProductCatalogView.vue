...
const MODEL_IMAGE_MAX_SIZE = 5 * 1024 * 1024
...
async function loadAll(keepSelection = true) {
  const loadingProducts = treeMode.value === 'products'
  treeLoading.value = loadingProducts
  error.value = ''
  try {
    if (loadingProducts && !keyword.value) {
      tree.value = await catalogTree('', { depth: 'category' })
...
async function refreshModelImage() {
  const modelId = model.value.id || catalogEditNode.value?.id || selected.value?.id
  if (!modelId) return
  await loadModel(modelId, version.value.id)
  modelImagePreview.value = String(model.value.image_url || model.value.image_object_key || '')
}
...
function openCreate(kind: CreateKind, editNode: RecordRow | null = null, defaults: Record<string, unknown> = {}) {
  ...
  if (kind === 'models') {
    const detailModel = String(model.value.id || '') === String(edit?.id || '') ? model.value : {}
    modelImagePreview.value = String(detailModel.image_url || edit?.image_url || detailModel.image_object_key || edit?.image_object_key || '')
    Object.assign(createForm, {
      seriesId: edit?.seriesId ?? (selected.value?.nodeType === 'SERIES' ? selected.value.id : ''),
...
async function onModelImageFile(event: Event) {
  const input = event.target as HTMLInputElement
  const file = input.files?.[0]
  input.value = ''
  if (!file) return
  const modelId = catalogEditNode.value?.id || model.value.id
  if (!modelId) {
    error.value = '请先保存型号后再上传图片'
    return
  }
  if (!MODEL_IMAGE_TYPES.includes(file.type)) {
    error.value = '仅支持 JPEG、PNG、WebP、GIF 图片'
    return
  }
  if (file.size > MODEL_IMAGE_MAX_SIZE) {
    error.value = '图片不能超过 5MB'
    return
  }
  modelImageSaving.value = true
  error.value = ''
  try {
    const saved = await catalogUploadModelImage(modelId, file)
    const savedModel = saved.model as RecordRow | undefined
    createForm.imageObjectKey = String(savedModel?.image_object_key || '')
    modelImagePreview.value = String(savedModel?.image_url || savedModel?.image_object_key || '')
    if (String(model.value.id || '') === String(modelId)) {
      detail.value = saved
      syncDrafts()
    }
    await loadAll(false)
  } catch (e) {
    error.value = e instanceof Error ? e.message : '型号图片上传失败'
  } finally {
    modelImageSaving.value = false
  }
}

async function clearModelImage() {
  const modelId = catalogEditNode.value?.id || model.value.id
  if (!modelId) return
  modelImageSaving.value = true
  error.value = ''
  try {
    const saved = await catalogClearModelImage(modelId)
    createForm.imageObjectKey = ''
    modelImagePreview.value = ''
    if (String(model.value.id || '') === String(modelId)) {
      detail.value = saved
      syncDrafts()
    ...
    error.value = e instanceof Error ? e.message : '型号图片清空失败'
  } finally {
    modelImageSaving.value = false
  }
}
...
<template>
  <section class="view-page catalog-page">
    <header class="view-head">
      <div><p class="eyebrow">DEVICE CATALOG</p><h1>产品目录与型号模板</h1></div>
    ...
    <div class="catalog-layout">
      <aside class="catalog-sidebar archive-sidebar-panel">
        ...
        <div class="archive-tree-list catalog-tree-list">
          <AppLoadingState v-if="treeLoading && !(treeMode === 'products' ? categoryTree : activeTree).length" />
          ...
          <p v-if="!(treeMode === 'products' ? categoryTree : activeTree).length && !treeLoading" class="empty-state">暂无目录数据</p>
        </div>
      </aside>
      ...
      <main v-if="modelDialog && isModel && model.id" class="catalog-main archive-main-panel catalog-detail-overlay">
        ...
        <AppLoadingState v-if="loading && !selected" />
        <template v-else-if="isModel && model.id">
          ...
          <div class="catalog-content">
            <section v-if="tab === 'basic'" class="catalog-basic-grid">
              <article class="panel catalog-model-image-panel">
                <label class="catalog-model-upload-frame" :class="{ filled: model.image_url, disabled: !canEdit || modelImageSaving }">
                  <input type="file" accept="image/jpeg,image/png,image/webp,image/gif" :disabled="!canEdit || modelImageSaving" @change="onModelImageFile">
                  <AppImage v-if="model.image_url || model.image_object_key" class="catalog-model-image" :src="model.image_url || model.image_object_key" :alt="String(model.model_name || '型号图片')" @retry="refreshModelImage" />
                  <span v-else class="catalog-model-upload-empty"><ImagePlus :size="34" /><b>{{ modelImageSaving ? '上传中...' : '上传型号图片' }}</b></span>
                </label>
            ...
            <section v-else-if="tab === 'attributes'" class="panel catalog-tab-panel">
              ...
              <div class="template-filter-bar">
                ...
                <AppSelect v-model="templateAttributeGroupFilter"><option value="">全部属性组</option><option v-for="group in templateAttributeGroupOptions" :key="group" :value="group">{{ group }}</option></AppSelect>
              </div>
...
.catalog-basic-grid{display:grid;grid-template-columns:minmax(210px,.58fr) minmax(0,1.42fr);grid-template-rows:minmax(150px,.72fr) minmax(220px,1fr);gap:12px;align-items:stretch}.catalog-basic-grid .catalog-form-panel{max-width:none}.catalog-model-image-panel{position:relative;grid-row:1 / 3;min-height:390px;padding:12px}.catalog-model-upload-frame{position:relative;width:100%;height:100%;min-height:366px;display:grid;place-items:center;overflow:hidden;border:1.5px dashed #9fb6cf;background:#fbfdff;color:#6d849d;cursor:pointer}.catalog-model-upload-frame input{position:absolute;inset:0;z-index:2;opacity:0;cursor:pointer}.catalog-model-upload-frame.filled{border-color:#d7e4f2;background:#fff}.catalog-model-upload-frame.disabled{cursor:default}.catalog-model-upload-frame.disabled input{pointer-events:none}.catalog-model-upload-frame img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;display:block}.catalog-model-upload-empty{display:grid;justify-items:center;gap:9px}.catalog-model-upload-empty b{font-size:13px;font-weight:600}.catalog-model-image-delete{position:absolute;right:19px;top:19px;z-index:8;width:30px;height:30px;display:grid;place-items:center;border:1px solid #f0c7c0;background:#fff;color:#c4564a;box-shadow:0 6px 18px #17375c28}.catalog-model-image-delete:hover{background:#fff2f0}.catalog-version-panel{min-width:0}.catalog-version-panel .catalog-version-list{max-height:170px;overflow:auto}.catalog-version-row{min-height:52px;display:grid;grid-template-columns:minmax(0,1fr) auto auto;align-items:center;gap:12px;padding:8px 10px;border-bottom:1px solid var(--border);background:#fff;color:var(--fg);text-align:left;cursor:pointer}.catalog-version-row:hover{background:#f5f9ff}.catalog-version-row.active{background:#eef6ff;color:#2367bd}.catalog-version-row.active b{color:#2367bd}.catalog-version-row.active em{font-weight:700}.catalog-version-row .icon-btn{width:28px;height:28px}@media(max-width:980px){.catalog-basic-grid{grid-template-columns:1fr;grid-template-rows:auto}.catalog-model-image-panel{grid-row:auto}.catalog-version-panel .catalog-version-list{max-height:none}}
