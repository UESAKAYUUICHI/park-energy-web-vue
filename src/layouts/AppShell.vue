...
const nav = computed<NavGroup[]>((): NavGroup[] => [
  { key: 'dashboard', label: '经营总览', icon: groupIcon('dashboard'), items: [item('dashboard')] },
  { key: 'deviceArchive', label: '设备档案', icon: groupIcon('deviceArchive'), items: [item('device-archive-org-tree'), item('device-archive-devices')] },
  { key: 'energy', label: '能源运营', icon: groupIcon('energy'), items: [item('monitor-realtime'), item('analysis-history'), item('analysis-quality')] },
  { key: 'alarm', label: '告警处置', icon: groupIcon('alarm'), items: [item('alarms-events'), item('alarms-workbench')] },
  { key: 'billing', label: '结算收款', icon: groupIcon('billing'), items: [item('billing-settlement'), item('billing-bills')] },
  { key: 'ops', label: '设备运维', icon: groupIcon('ops'), items: [item('access-diagnostic'), item('access-control'), item('access-commands')] },
  { key: 'system', label: '系统管理', icon: groupIcon('system'), sections: [
